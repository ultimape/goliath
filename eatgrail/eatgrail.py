...
def parse_twitter_export_for_ids(EXPORT_LOCATION):
    ...
    tweets_by_month = load_tweet_export_index(EXPORT_LOCATION)
    ...
    total_tweet_count, total_retweet_count, total_quotetweet_count = \
        extract_tweets_from_tweet_export(tweets_by_month, EXPORT_LOCATION)
...
def load_tweet_export_index(EXPORT_LOCATION):
...
def extract_tweets_from_tweet_export(tweets_by_month, EXPORT_LOCATION):
    total_tweet_count = 0
    ...
    try:
        for date in tweets_by_month:
            try:
                ...
                print("    Parsing %s " % data_filename, end='')
