...
Under the `eatgrail` directory is a python module called `eatgrail.py` that can be ran directly as a script. This module can be told about your twitter acrchive directory, and it will harvest tweet IDs for you.
...
> *Note*: Tweet IDs are "deduplicated". So, say for example you have a tweetstorm where you've replied to and often quotetweet yourself. Since these tweet IDs already show up under `...tweet_ids.txt`, they will not be present within the `...retweeted_ids.txt`, `...quotetweet_ids.txt`, or `...replyto_ids.txt` files. However the `...retweet_id.txt` will still contain the proxy entries that retweeting generates (nessisary for recreating a feed).
...
This is a rough guideline of the direction the project is headed.  
It is an evolving list. *Not promises*.  

**Manual Exploration:**  
*Stuff We've figured out how to do manually.*  
- [x] export tweets 
- [x] extract
...
*Stuff we need to write code to do.*  
