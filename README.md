...
`<id>_<username>_export_tweet_ids.txt` contains IDs of tweets you've personally tweeted out.  
`<id>_<username>_export_retweet_ids.txt` contains IDs of tweet proxies associated with each of your retweets.  
`<id>_<username>_export_retweeted_ids.txt` contains IDs of the actual retweeted tweets.  
`<id>_<username>_export_quotetweet_ids.txt` contains IDs of any tweets you've linked to ('quoted') to in a tweet.  
`<id>_<username>_export_replyto_ids.txt` contains IDs of a tweet that is listed as having been replied to.  
`<id>_<username>_export_all_ids.txt` contains every ID of all the other files (mostly for convenience factor).
`<id>_<username>_export_user_details.txt` contains information about the account who's archive it was.

The `<id>` and `<username>` parts will be replaced with whatever 
...
**Eventually twarc will be baked into the main goliath.py to automate it all**, but it was important to *release this tool in it's current form* for people to tinker with.
...
- [ ] Automatically download data from Twitter
  - [x] Tweet Data+Metadata
  - [x] Avatars
  - [ ] Tweet Images
  - [ ] Complete Videos
